<div id="execution-monitor-widget" x-data="monitorWidget()" x-cloak 
     style="--lorapok-primary: {{ config('execution-monitor.theme.primary', '#667eea') }}; --lorapok-secondary: {{ config('execution-monitor.theme.secondary', '#764ba2') }}; --lorapok-accent: {{ config('execution-monitor.theme.accent', '#ef4444') }};">
    <!-- Main Larvae Button -->
    <button @click="toggleModal()" class="lorapok-btn" style="position:fixed;bottom:20px;right:20px;z-index:9999;width:80px;height:80px;border:none;background:linear-gradient(135deg, var(--lorapok-primary) 0%, var(--lorapok-secondary) 100%);border-radius:50%;cursor:pointer;box-shadow:0 8px 20px rgba(102,126,234,0.4);display:flex;align-items:center;justify-content:center;font-size:42px;transition:all 0.3s ease">
        🐛
        <span x-show="hasAlerts" style="position:absolute;top:-5px;right:-5px;width:28px;height:28px;background:var(--lorapok-accent);border-radius:50%;color:white;font-size:16px;font-weight:bold;display:flex;align-items:center;justify-content:center;animation:pulse 1.5s infinite;border:3px solid white">!</span>
    </button>

    <!-- Floating actions removed: buttons moved into the main modal header for a professional layout -->

    <!-- Developer Info Modal -->
    <div x-show="showDevInfo" x-transition @click.away="showDevInfo = false" class="fixed inset-0 z-[11000] flex items-center justify-center" style="display:none;position:fixed;top:0;left:0;right:0;bottom:0;z-index:11000;display:flex;align-items:center;justify-content:center">
        <div class="absolute inset-0 bg-gray-900/75 backdrop-blur-md" style="position:absolute;top:0;left:0;right:0;bottom:0;background:rgba(17,24,39,0.75);backdrop-filter:blur(8px)"></div>
        <div class="relative rounded-[2rem] shadow-2xl p-8 max-w-md w-full border border-white/20" style="background: linear-gradient(135deg, var(--lorapok-primary), var(--lorapok-secondary)); border-radius: 2rem; position:relative; z-index:11001">
    ...
    <!-- Main Monitor Modal -->
    <div x-show="isOpen" x-transition class="fixed inset-0 z-[10000] flex items-center justify-center" style="position:fixed;top:0;left:0;right:0;bottom:0;z-index:10000;display:flex;align-items:center;justify-content:center" x-cloak>
        <div class="absolute inset-0 bg-gray-900 bg-opacity-75 backdrop-blur-md" @click="closeModal()" style="position:absolute;top:0;left:0;right:0;bottom:0;background:rgba(17,24,39,0.75);backdrop-filter:blur(8px)"></div>
        <div class="bg-white rounded-[2rem] shadow-2xl" style="background:#fff;border-radius:2rem;box-shadow:0 25px 50px -12px rgba(0,0,0,0.25);width:100%;max-width:75rem;height:85vh;max-height:850px;display:flex;flex-direction:column;overflow:hidden;position:relative;z-index:10001">
            <div class="bg-gradient-to-r from-purple-600 to-blue-600 px-6 py-4 flex items-center justify-between" style="flex-shrink:0; background: linear-gradient(to right, var(--lorapok-primary), var(--lorapok-secondary))">
                <div>
    ...
    <div x-show="openSettings" x-transition class="fixed inset-0 z-[10500] flex items-center justify-center" style="display:none;pointer-events:auto;position:fixed;top:0;left:0;right:0;bottom:0;z-index:10500;display:flex;align-items:center;justify-content:center">
        <!-- Backdrop -->
        <div class="absolute inset-0 bg-gray-900 bg-opacity-75 backdrop-blur-md" @click="openSettings = false" style="position:absolute;top:0;left:0;right:0;bottom:0;background:rgba(17,24,39,0.75);backdrop-filter:blur(8px)"></div>
        
        <!-- Modal Content -->
        <div class="relative text-white rounded-[2rem] shadow-2xl p-8 w-full max-w-md border border-white/20" style="background: linear-gradient(135deg, var(--lorapok-primary), var(--lorapok-secondary)); border-radius: 2rem; position:relative; z-index:10501">
            <!-- Close Button -->
